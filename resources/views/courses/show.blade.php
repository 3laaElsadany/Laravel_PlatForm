        ...
        <aside class="lg:col-span-1">
            <div class="card-surface sticky top-24 space-y-6 p-6 shadow-card">
                ...
                <div class="rounded-xl bg-slate-50 p-4 text-xs leading-relaxed text-slate-600 dark:bg-slate-800/80 dark:text-slate-300">
                    {{ __('Enrollment happens after a successful payment.') }}
                </div>
                ...
                @auth
                    @if ($enrolled)
                        ...
                        @endcan
                    @elseif ($trialExpired)
                        <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">{{ __('Your free trial has expired. Please pay to continue accessing the course.') }}</div>
                        <a href="{{ route('courses.checkout.show', $course) }}" class="btn-primary w-full text-center mt-4">{{ __('Pay & enroll') }}</a>
                    @else
                        @can('enroll', $course)
