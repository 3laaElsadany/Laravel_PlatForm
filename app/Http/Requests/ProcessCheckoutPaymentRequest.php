...
class ProcessCheckoutPaymentRequest extends FormRequest
{
    ...
    public static function availablePaymentMethods(): array
    {
        ...
        if ((bool) config('payments.trial_enabled')) {
            $user = auth()->user();
            $course = request()->route('course');

            $showTrial = true;
            if ($user !== null && $course instanceof Course) {
                $showTrial = ! $user->enrollments()->where('course_id', $course->id)->exists();
            }

            if ($showTrial) {
                array_unshift($methods, 'trial');
            }
        }
